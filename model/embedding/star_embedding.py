...
from model.embedding.graph_model import GAT, SoftmaxAttention
...
class STAREmbedding(nn.Module):
    ...
    def __init__(
        ...
        # Se define un GAT independiente por cada grafo (SDG, TTG, STG)
        ...
        # Mecanismo de fusión de los tres embeddings en uno solo
    ...
    def _load_graphs(self):
        ...
        for name in self.GRAPH_NAMES:
            ...
            # Construye grafo final
            g = dgl.graph((src, dst), num_nodes=nodes_df.shape[0])
            # Conectar cada nodo consimo mismo
            g = dgl.add_self_loop(g)

            # Pesos de aristas: aristas originales y 1.0 en los self-loops
    ...
    def forward(self):
        ...
        num_nodes   = self.graphs[0].num_nodes() # numero de nodos
        node_ids    = torch.arange(num_nodes, device=self.device) # lista de los node ids
        ...
        # fused:   [num_nodes, out_dim] -> un embedding por nodo
...
if __name__ == "__main__":
    ...
    PROPATH  = f"{PROJECT_DIR}/data/graphs/NYC"
    FEA_DIM  = 32
    HID_DIM  = 32
    OUT_DIM  = 32
    LAYER_NUM = 2
